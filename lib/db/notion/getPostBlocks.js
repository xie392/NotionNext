...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  ...
  try {
    ...
    // 检测是否是 429 Too Many Requests 错误
    const is429Error = e?.message?.includes('429') || e?.status === 429
    if (is429Error) {
      console.warn('[API限流] 触发 429 限流，等待后重试...')
      // 429 错误时增加等待时间，使用指数退避
      const waitTime = (4 - retryAttempts) * 2000 // 重试次数越少，等待时间越长
